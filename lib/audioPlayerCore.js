...
var audio = _exenv.canUseDOM ? new Audio() : null;
var tracks = void 0;
...
var addTracks = exports.addTracks = _exenv.canUseDOM ? function (newTracks) {
...
var play = exports.play = _exenv.canUseDOM ? function () {
    if (isPlaying()) {
        audio.pause();
        stopTimer();
...
var stop = exports.stop = _exenv.canUseDOM ? function () {
    audio.pause();
    audio.currentTime = 0;
    stopTimer();
...
var seek = exports.seek = _exenv.canUseDOM ? function (seconds) {
    audio.currentTime = seconds;
    updateListeners(UpdateTypes.SEEK);
} : noop;

var next = exports.next = _exenv.canUseDOM ? function () {
    currentTrackIndex = currentTrackIndex + 1 >= tracks.length ? 0 : currentTrackIndex + 1;
    switchTrack();
} : noop;

var prev = exports.prev = _exenv.canUseDOM ? function () {
    currentTrackIndex = currentTrackIndex - 1 < 0 ? tracks.length - 1 : currentTrackIndex - 1;
    switchTrack();
} : noop;

var goto = exports.goto = _exenv.canUseDOM ? function (number) {
    currentTrackIndex = number - 1;
    switchTrack();
} : noop;

var gotoAndPlay = exports.gotoAndPlay = _exenv.canUseDOM ? function (number) {
    goto(number);
    if (!isPlaying()) {
        play();
    }
...
var turnOnAutoplay = exports.turnOnAutoplay = _exenv.canUseDOM ? function () {
    audio.autoplay = true;
} : noop;
